...
export const chatAPI = {
  startSession: (expertId, duration) =>
    api.post('/chat/start', { expertId, duration }).then(res => res.data),
...
export const adminAPI = {
  ...
  updateSession: (id, data) =>
    api.put(`/admin/sessions/${id}`, data).then(res => res.data),
  
  deleteSession: (id) =>
    api.delete(`/admin/sessions/${id}`).then(res => res.data),
  ...
  markInquiryRead: (id) =>
    api.put(`/admin/inquiries/${id}/mark-read`).then(res => res.data),
...
export const profileAPI = {
  updateProfile: (data) => {
    console.log('Making profile update request to:', `${API_BASE_URL}/auth/profile/update`);
  ...
  changePassword: (data) => {
    console.log('Making password change request to:', `${API_BASE_URL}/auth/profile/change-password`);
    return api.put('/auth/profile/change-password', data)
      .then(res => {
        console.log('Password change success:', res.data);
        return res.data;
      })
      .catch(error => {
        console.error('Password change error:', error.response?.status, error.response?.data);
        throw error;
      });
  },
};
